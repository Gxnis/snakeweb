...
- **Authentification utilisateur** : Inscription et connexion avec email/mot de passe
- **Sauvegarde des scores** : Vos scores et statistiques sont sauvegardés
- **Tableau des morts** : Suivi des dernières morts avec statistiques
- **Badge vérifié** : Badge pour les utilisateurs vérifiés
- **Sélection de couleur** : Choisissez votre couleur de serpent

## Technologies

- **Backend** : Node.js, Express, Socket.io, SQLite3, bcrypt, jsonwebtoken
...
3. **Variables d'environnement (optionnelles)**
   ...
   - `JWT_SECRET` : Clé secrète pour les tokens JWT (générée automatiquement si non définie)
   - `NODE_ENV` : Défini automatiquement sur 'production' par Render
...
### Erreur de connexion au serveur (login/inscription)

- Sur Render, l'API utilise des URLs relatives (pas localhost)
- Vérifiez que CORS est configuré correctement dans `server/index.js`
- Vérifiez les logs Render pour les erreurs de base de données
...
- **Important** : SQLite stocke les données dans le système de fichiers
- Sur Render, les données peuvent être perdues après un redéploiement
- Pour la production, envisagez d'utiliser PostgreSQL (disponible sur Render)
- Pour utiliser PostgreSQL, modifiez `server/database.js` pour utiliser `pg` au lieu de `sqlite3`
